...
const deleteQuery = () => {
    ...
    deleteBtns.forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            await fetch(`http://localhost:3000/admin/queries/${btn.id}`, {
                method: 'DELETE',
            })
...
const respondQuery = async () => {
    ...
    let res = await (await fetch(`http://localhost:3000/admin/queries/${sendResBtn.id}`)).json();
    ...
    sendResBtn.addEventListener('click', async (e) => {
        ...
        await fetch(`http://localhost:3000/admin/queries/${sendResBtn.id}`, {
            method: 'PATCH',
